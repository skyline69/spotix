    ...
    async fn load_remote_track(
        ...
        loop {
            ...
            let key = match self.session.audio_key().request(track_id, file_id).await {
                Ok(key) => Some(key),
                Err(e) => {
                    // If the file is cached and we can't get a key, the cached blob
                    // might be corrupted or stale. Evict it and retry from network.
                    if is_cached {
                        warn!("Unable to load key for cached file, evicting cache and retrying: {e}");
            ...
            let stream_position_ms = match decoder.seek(position_ms) {
                ...
                Err(e) if is_cached => {
                    warn!(
                        "Seek failed on cached audio file: {e}. Evicting cache and retrying."
                    );
                    if let Some(cache) = self.session.cache() {
                        let _ = cache.remove_file(file_id);
                    }
